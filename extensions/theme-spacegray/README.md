Run `yarn ts-node extensions/theme-spacegray/themes/generate.ts` to generate the themes.

You can also use this if you have `chokidar-cli` installed:

```
chokidar "extensions/theme-spacegray/themes/**/*.ts" -c "yarn ts-node extensions/theme-spacegray/themes/generate.ts"
```
...
```
chokidar "extensions/theme-spacegray/themes/**/*.ts" -c "yarn ts-node extensions/theme-spacegray/themes/write-into-user-settings.ts ~/Library/Application\ Support/code-oss-dev/User/settings.json"
```
